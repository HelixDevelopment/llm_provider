...
// pinnedKeys maps a provider key to the three literal environment-variable
// names it must produce. An empty string means "this adapter does not resolve
// that setting", and is not asserted.
...
func TestKeyProducesTheLiteralOperatorFacingName(t *testing.T) {
	...
	for provider, want := range pinnedKeys {
