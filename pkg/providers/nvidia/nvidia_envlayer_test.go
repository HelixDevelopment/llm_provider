...
// NvidiaModel }` had nothing behind it, so a vendor retiring or rate-capping
...
func TestDefaultModelIsOverridableFromTheEnvironment(t *testing.T) {
	key := settings.Key("nvidia", settings.SuffixModel)

	t.Setenv(key, "")
	assert.Equal(t, NvidiaModel, NewNvidiaProvider("k", "", "").model,
		"with nothing set the compiled fallback must still apply")

	t.Setenv(key, "operator-chosen-model")
	assert.Equal(t, "operator-chosen-model", NewNvidiaProvider("k", "", "").model,
		"%s did not reach the constructor - the default is still frozen", key)
}

func TestExplicitModelOutranksTheEnvironment(t *testing.T) {
	t.Setenv(settings.Key("nvidia", settings.SuffixModel), "from-env")
	assert.Equal(t, "caller-model", NewNvidiaProvider("k", "", "caller-model").model,
