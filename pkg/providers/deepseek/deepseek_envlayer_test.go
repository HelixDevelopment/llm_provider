...
// DeepSeekModel }` had nothing behind it, so a vendor retiring or rate-capping
...
func TestDefaultModelIsOverridableFromTheEnvironment(t *testing.T) {
	key := settings.Key("deepseek", settings.SuffixModel)

	t.Setenv(key, "")
	assert.Equal(t, DeepSeekModel, NewDeepSeekProvider("k", "", "").model,
		"with nothing set the compiled fallback must still apply")

	t.Setenv(key, "operator-chosen-model")
	assert.Equal(t, "operator-chosen-model", NewDeepSeekProvider("k", "", "").model,
		"%s did not reach the constructor - the default is still frozen", key)
}

func TestExplicitModelOutranksTheEnvironment(t *testing.T) {
	t.Setenv(settings.Key("deepseek", settings.SuffixModel), "from-env")
	assert.Equal(t, "caller-model", NewDeepSeekProvider("k", "", "caller-model").model,
