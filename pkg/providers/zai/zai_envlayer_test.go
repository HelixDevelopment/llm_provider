...
// DefaultZAIModel }` had nothing behind it, so a vendor retiring or rate-capping
...
func TestDefaultModelIsOverridableFromTheEnvironment(t *testing.T) {
	key := settings.Key("zai", settings.SuffixModel)

	t.Setenv(key, "")
	assert.Equal(t, DefaultZAIModel, NewZAIProvider("k", "", "").model,
		"with nothing set the compiled fallback must still apply")

	t.Setenv(key, "operator-chosen-model")
	assert.Equal(t, "operator-chosen-model", NewZAIProvider("k", "", "").model,
		"%s did not reach the constructor - the default is still frozen", key)
}

func TestExplicitModelOutranksTheEnvironment(t *testing.T) {
	t.Setenv(settings.Key("zai", settings.SuffixModel), "from-env")
	assert.Equal(t, "caller-model", NewZAIProvider("k", "", "caller-model").model,
